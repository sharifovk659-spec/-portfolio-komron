...
export default function Preloader() {
  ...
  useEffect(() => {
    ...
    gsap.timeline()
      ...
      .fromTo(".preloader-letter", { opacity: 0, y: 40, rotateX: -90 }, { opacity: 1, y: 0, rotateX: 0, duration: 0.6, stagger: 0.15, ease: "back.out(2)" }, "-=0.5")
      .fromTo(".preloader-logo-box", { scale: 0.5, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.8, ease: "back.out(1.7)" }, "-=0.8");
  }, [loading]);
    ...
    <AnimatePresence>
      {loading && (
        <motion.div exit={{ opacity: 0 }} transition={{ duration: 0.7 }} className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-[#050508]">
          ...
          <div className="preloader-logo-box relative flex h-28 w-28 items-center justify-center rounded-2xl sm:h-32 sm:w-32" style={{ background: "linear-gradient(135deg, rgba(0,212,255,0.15), rgba(168,85,247,0.15))", border: "1px solid rgba(255,255,255,0.15)", boxShadow: "0 0 60px rgba(0,212,255,0.35)" }}>
            {"SK".split("").map((letter, i) => (
              <span key={i} className="preloader-letter font-display text-4xl font-bold gradient-text sm:text-5xl">{letter}</span>
            ))}
          </div>
